...
# MAGIC %md
# MAGIC ## 2. ValuationAgent with Tracing
# MAGIC
# MAGIC The `ValuationAgent` class from `valuation_curator.agent` provides:
...
# MAGIC
# MAGIC ```python
# MAGIC class ValuationAgent(ResponsesAgent):
...
# MAGIC %md
# MAGIC ## 3. Create ValuationAgent Instance
# MAGIC
# MAGIC The agent automatically gets MCP tools for:
# MAGIC - Vector Search (search valuation documents)
...
logger.info("✓ ValuationAgent created with MCP tools:")
...
if len(session_traces_df) > 0:
    ...
    if simple_cols:
        display(session_traces_df[simple_cols].head())  # noqa: F821
    else:
